...
int findMergeNode(SinglyLinkedListNode* head1, SinglyLinkedListNode* head2) {
    ...
    // Redirect each pointer to the head of the other list when it reaches the end.
    // They will meet at the merge node after traversing at most (length1 + length2) nodes.
...
int main()
{
    FILE* fptr = fopen(getenv("OUTPUT_PATH") ? getenv("OUTPUT_PATH") : "output.txt", "w");
    ...
    for (int tests_itr = 0; tests_itr < tests; tests_itr++) {
        ...
        // Note: Freeing is a bit tricky here because of the merge.
        // Usually, we'd free the lists, but here they share nodes.
    }
